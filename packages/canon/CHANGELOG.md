...
## 0.14.0 — «Примеры v2 · полноразмерные сайты за „Посмотреть сайт“» · 2026-07-23

> **MINOR (0.x) — ADDITIVE, без breaking.** Секция 03 лендинга v5 «Примеры» больше не открывает «Посмотреть сайт» узкой 480-px колонкой с той же миниатюрой — теперь это **5 полноразмерных сайтов-примеров** в **takeover-просмотрщике во всю ширину** с навигацией назад и всегда-доступным CTA. Закрытое состояние `V5_Examples` идентично 0.13.0 (с точностью до новых `data-example-*` хуков). Источник: `uploads/CANON_V5_EXAMPLE_SITES_TZ.md` + `_EXPORT_TZ.md`, прототип `Витрина v5 - canon.html` / `canon-v5/v5-sites{,-kit}.jsx`. Все прочие модули — byte-identical 0.13.0 (§8).

### 1. Что нового

- **5 полноразмерных сайтов** (`FullComp`): `NailSite`, `BarberSite`, `SkinSite`, `BrowSite`, `ColoristSite` — развитые версии миниатюр `EXAMPLES`. Первый экран каждого узнаваемо совпадает с карточкой карусели (палитра/типографика/персонаж/анатомия 1:1), ниже сайт «разворачивается»: полный прайс (6–10 позиций; у brows короче — легенда «минимал»), галерея работ (4–7 фото), отзывы (3, у skin 3 + один «длинный»), «о мастере» с фактами, запись + контакты/часы, футер с watermark **«Сделано на Самосайте»** (некликабельный — реализм free-плана).
- **`V5_SiteViewer` переписан в takeover** во всю рабочую ширину (desktop), фуллскрин (mobile). Sticky-шапка: кнопка назад **«Все примеры»** (`data-example-back`, на mobile «Примеры»), псевдо-домен, CTA **«Собрать такой же за 2 часа»** (desktop — в шапке; mobile — sticky-низ, зона большого пальца). Esc / кнопка назад / × закрывают; focus-trap + возврат фокуса на открывшую кнопку сохранены; `prefers-reduced-motion: reduce` — без анимации входа.
- **fix (scroll-lock):** `body.is-locked` **впервые получил CSS-правило** `{overflow:hidden}` в `V5_CSS` (раньше класс вешался, но правила не было — фон скроллился под оверлеем). Плюс `overscroll-behavior: contain` на скроллере просмотрщика.

### 2. Публичный API (additive)

```ts
interface ExampleEntry { /* …как в 0.13.0… */ FullComp?: React.ComponentType; }  // НОВОЕ поле (опционально)
interface V5_ExamplesProps {
  layout?: 'carousel' | 'zigzag'; onIntake?: IntakeCb;
  viewerId?: string | null;                        // НОВОЕ: controlled open
  onViewerChange?: (id: string | null) => void;    // НОВОЕ: клик «Посмотреть» / закрытие
}
// V5_SiteViewer({ data, onClose, onClaim }) — рендерит data.FullComp ?? data.Comp (фоллбэк на миниатюру).
```

- Zero-prop `<V5_Examples />` — внутренний `useState`, карусель как 0.13.0; клик открывает полноразмерник. Controlled — когда передан `viewerId` (консьюмер держит стейт + history). Неизвестный / протухший `viewerId` — не рендерит ничего, не падает. `FullComp` заполнен у всех 5 записей; тип опционален, чтобы не ломать внешних консьюмеров `ExampleEntry`.
- Внутри канона нет `fetch`/`history`/`window.*`-побочек/`Date.now` (только легитимные `window.matchMedia`/`innerWidth` для адаптива, как в базе). Историю/скролл-рестор/Метрику делает консьюмер.

### 3. Раскладка файлов

- `src/landing/v5-sites.tsx` — **НОВЫЙ.** 5 `FullComp` + `V5_SiteViewer`/`V5_SiteViewerProps`. `'use client'`.
- `src/landing/v5-sites-kit.tsx` — **НОВЫЙ, внутренний.** Примитивы (`Img`/`FakeCTA`/`Stars`/`SiteHead`/`SectionHead`/`PriceList`/`Gallery`/`Reviews`/`About`/`Contacts`/`Watermark` + палитры `Pn/Pb/Ps/Pr/Pc`). **Не** ре-экспортируется из index — только относительный импорт из `v5-sites.tsx`. `'use client'`.
- `src/landing/v5.tsx` — дифф **строго** в блоке Examples/SiteViewer: `ExampleEntry` (+`FullComp?`), `EXAMPLES` (импорт+привязка `FullComp` статически), `PreviewWithCTA` (`data-example-open`), `V5_Examples` (controlled), `V5_CSS` (+`body.is-locked`+`@keyframes v5-viewer-in`), старый `V5_SiteViewer` удалён (перенесён в `v5-sites.tsx`), + одна строка `export * from './v5-sites'`. **Hero и все прочие секции byte-identical.** Полный файл + `v5.tsx.diff` в корне пакета.
- `src/tokens.ts` — только версия-коммент → 0.14.0. `package.json` — `0.14.0` + description-маркер; **`exports` не менялся** (новые файлы в чанке `./landing`).

### 4. Реестр копи (единственный источник правок; типографика по `specs/04_typography.md`)

**Хром просмотрщика:** назад «Все примеры» / «Примеры» (mobile) · CTA «Собрать такой же за 2 часа →» · aria «Закрыть просмотр» · псевдо-домен `{domain}.samosite.online`.

**1 · nails — Студия Анны (Екатеринбург, Cormorant):**

| Блок | Строки |
|---|---|
| Идентичность | «Студия Анны» · «Екатеринбург · маникюр» · навигация: Услуги / Работы / Отзывы / Контакты · CTA «Записаться в Telegram» |
| Герой | кикер «Аппаратный маникюр · без спешки» · H1 «Аппаратный маникюр, держится 3 недели» |
| О мастере | «Работаю одна, без спешки — один клиент в час» + 2 абзаца; факты: 8 лет / у кресла · 3 недели / носибельность покрытия · 1 час / на одного клиента |
| Прайс «Цены за июль» → «Прайс без догоняющих доплат» | Маникюр + однотонное покрытие 2 400 ₽ · Маникюр без покрытия 1 500 ₽ · Снятие чужого покрытия 400 ₽ · Дизайн на 2 ногтя 300 ₽ · Френч / лунки 500 ₽ · Укрепление акригелем 600 ₽ · Втирка / хром 350 ₽ · Парафинотерапия рук 700 ₽ · SPA-уход с массажем 900 ₽ |
| Отзывы | Олеся Н. (Яндекс) · Марина К. (Яндекс) · Дарья П. (2ГИС) |
| Контакты | Екатеринбург, ул. Малышева 51, БЦ «Высоцкий» · Пн–Сб 10:00–20:00, Вс по договорённости · @anna_nails · note «Ответ обычно в течение часа» |
| Футер | «Студия Анны» · «Екатеринбург · маникюр» · «Сделано на Самосайте» |

**2 · barber — Барбершоп «Фёдор» (Москва, Oswald):**

| Блок | Строки |
|---|---|
| Идентичность | «БАРБЕРШОП „ФЁДОР“» · «Москва · с 2018» · Прайс / Мастера / Работы / Адрес · CTA «Записаться» |
| Герой | кикер «Только мужские стрижки · Москва» · H1 «Стрижка + борода за 45 минут» · «Виски бесплатно… Кофе за счёт заведения» |
| Прайс | Мужская стрижка 2 200 ₽ · Стрижка + борода 3 000 ₽ · Детская (до 12 лет) 1 500 ₽ · Камуфляж седины 1 200 ₽ · Моделирование бороды 1 600 ₽ · Опасной бритвой 1 800 ₽ · Королевское бритьё 2 200 ₽ |
| Мастера | «Три мастера, стрижём с 2018 года» + 2 абзаца; факты: 6 лет / на одном месте · 3 / мастера в смене · 45 мин / стрижка + борода |
| Отзывы | Антон К. (Яндекс.Карты) · Илья Р. (Яндекс.Карты) · Сергей В. (2ГИС) |
| Контакты | Москва, Большая Дмитровка 17 · ежедневно 10:00–22:00 · @fedor_barber · «Можно без записи» |
| Футер | «Барбершоп „Фёдор“» · «Москва · с 2018» · «Сделано на Самосайте» |

**3 · skin — Кабинет Юлии (Казань, Spectral):**

| Блок | Строки |
|---|---|
| Идентичность | «Кабинет Юлии» · «Косметолог-эстетист · Казань» · Услуги / Работы / О себе / Запись · CTA «Записаться» |
| Герой | кикер «Косметолог · медицинское образование» · H1 «Сначала смотрим кожу, потом выбираем уход» |
| До/после | «Реальные результаты без фотошопа»; пары: Комбинированная чистка · 1 процедура · Курс пилингов · 4 процедуры |
| Прайс | Комбинированная чистка 3 200 ₽ · Ультразвуковая чистка 2 400 ₽ · Уходовая процедура от 2 000 ₽ · Пилинг по показаниям от 2 400 ₽ · Курс пилингов (4) 8 400 ₽ · Уход от купероза от 2 800 ₽ · Первичная диагностика бесплатно · Ведение по плану по договорённости |
| О себе | «Работаю по показаниям, а не по прайсу» + 2 абзаца; факты: 7 лет / в косметологии · мед. / образование · 4,9 ★ / 2ГИС и Яндекс |
| Отзывы | Ольга В. (2ГИС, длинный) · Регина С. (Яндекс) · Алина Т. (Яндекс) |
| Контакты | Казань, ул. Профсоюзная 12 · Вт–Сб 09:00–19:00 · @yulia_skin · «Первичная диагностика — бесплатно» |
| Футер | «Кабинет Юлии» · «Казань · косметолог» · «Сделано на Самосайте» |

**4 · brows — Brow Bar Соль (Сочи, Playfair Display; легенда «минимал» — короткий сайт, много воздуха):**

| Блок | Строки |
|---|---|
| Идентичность | «Brow Bar Соль» · «Сочи · брови и ресницы» · Услуги / Работы / Запись · CTA «Записаться» |
| Герой | кикер «Brow Bar Соль · Сочи» · H1 «Брови по вашей форме лица» · «Две услуги, ничего лишнего…» |
| Прайс «Две услуги и прозрачные цены» | Коррекция + окрашивание 1 400 ₽ · Ламинирование бровей 2 200 ₽ · Коррекция формы 700 ₽ · Снятие / переоформление 500 ₽ |
| Отзыв | Марина В. (5,0 · Яндекс) — центрированный |
| Контакты | Сочи, ул. Навагинская 9 · ежедневно 10:00–20:00 · @sol_brows · «Одна услуга — один час только для вас» |
| Футер | «Brow Bar Соль» · «Сочи · брови и ресницы» · «Сделано на Самосайте» |

**5 · colorist — Колорист Кира (Москва, Cormorant; анатомия «прайс-меню» — прайс главный блок):**

| Блок | Строки |
|---|---|
| Идентичность | «Колорист Кира» · «Москва · сложное окрашивание» · Прайс / Работы / О себе / Запись · CTA «Записаться» |
| Герой | кикер «Колорист · airtouch, балаяж, тон в тон» · H1 «Прайс без звёздочек» · «Цена финальная…» |
| Меню · Окрашивание | Сложное окрашивание от 6 500 ₽ · Окрашивание в один тон от 3 800 ₽ · Тонирование 3 200 ₽ · Осветление корней от 3 500 ₽ · Смывка / коррекция цвета от 2 800 ₽ |
| Меню · Уход | Ботокс для волос 4 500 ₽ · Кератиновое выпрямление от 5 500 ₽ · Уход за цветом 1 500 ₽ |
| Меню · Стрижка и укладка | Женская стрижка 2 500 ₽ · Укладка 1 800 ₽ · Стрижка + укладка 3 500 ₽ |
| О себе | «Осветляю бережно, цвет держу до тона» + 2 абзаца; факты: 12 лет / в цвете · 1200+ / окрашиваний · 4,9 ★ / Яндекс |
| Отзывы | Настя Л. (Яндекс) · Вика М. (Яндекс) · Лена Ж. (2ГИС) |
| Контакты | Москва, Столешников пер. 8, салон «Лак» · Вт–Вс 10:00–21:00 · @kira_color · «бесплатная консультация по фото» |
| Футер | «Колорист Кира» · «Москва · окрашивание» · «Сделано на Самосайте» |

**`note`-строки карточек карусели — НЕ менялись** (byte-identical 0.13.0). Выбран путь «легенды важнее контент-минимума» (осн. ТЗ §2): brows остаётся коротким/воздушным, barber держится на плакатной типографике — полноразмерники не ломают легенду, поэтому переписывать `note` не потребовалось.

### 5. Ассеты (фикстуры)

Локальные: `img/nails.png` (герой + галерея nails), `img/manicure.jpg` (nails «о мастере»). **Новых локальных файлов не добавлено.**

Unsplash-id новых фото (деградация `Img` при мёртвом id — штриховка + подпись, как в базе):

- **nails:** `photo-1604654894610-df63bc536371`, `-1632345031435-8727f6897d53`, `-1519014816548-bf5fe059798b`, `-1610992015732-2449b76344bc`, `-1607779097040-26e80aa78e66`
- **barber:** `photo-1503951914875-452162b0f3f1` (= миниатюра), `-1599351431202-1e0f0137899a`, `-1621605815971-fbc98d665033`, `-1585747860715-2ba37e788b70`, `-1622286342621-4bd786c2447c`
- **skin:** `photo-1570172619644-dfd03ed5d881` (= миниатюра), `-1616394584738-fc6e612e71b9` (= миниатюра), `-1512290923902-8a9f81dc236c`, `-1598440947619-2c35fc9aa908`, `-1559599101-f09722fb4948`
- **brows:** `photo-1616683693504-3ea7e9ad6fec` (= миниатюра), `-1583001931096-959e9a1a6223`, `-1594744803329-e58b31de8bf5`, `-1526045478516-99145907023c`
- **colorist:** `photo-1580618672591-eb180b1a973f` (= миниатюра), `-1562322140-8baeececf3df`, `-1595475884562-073c30d45670`, `-1519699047748-de8e457a634e`, `-1492106087820-71f1a00d2b11`

Замена Unsplash-фикстур на CDN — прод-задача vitrina (out of scope канона).

### 6. data-* хуки (цели Метрики)

- корень просмотрщика — `data-example-site="<id>"` (nails/barber/skin/brows/colorist);
- кнопка «Посмотреть сайт» в карточке — `data-example-open="<id>"` (НОВЫЙ);
- кнопка назад — `data-example-back` (НОВЫЙ);
- CTA сборки — `data-entry="example-<id>"` — **без изменений** (уже подключён к `form_open`).

События (`example_site_open {example}` / `example_site_back`) шлёт консьюмер.

### 7. Миграция консьюмеру

- Без изменений кода — работает как раньше, но «Посмотреть сайт» теперь открывает полноразмерник (это фича, не регресс).
- Controlled-режим (для browser Back / deep-link): держать `viewerId` в стейте страницы, `onViewerChange(id)` → `history.pushState`; фактическое закрытие — через `popstate`, UI-закрытие → `history.back()` (контракт — приложение осн. ТЗ). Метрику вешать на `data-example-*` через `ssTrack`.

### 8. Byte-identical 0.13.0 (не трогалось)

`src/intake/*`, `src/customer/*`, `src/feedback/*`, `src/admin-core/*`, `src/admin-demo/*`, `src/admin-ops/*`, `src/auth/*`, `src/source/*`, `src/presets/*`, `src/primitives/*`, `src/styles.css`, `src/CanonStyles.tsx`, `src/tailwind-preset.ts`, `src/index.ts`, `README.md`, `docs/*`. `src/landing/index.tsx` (v3-секции) — byte-identical; `src/tokens.ts` — только версия-коммент. Значения токенов «Фарфор и лак» не менялись.

### Vitrina-side (this vendoring PR)

Вендоринг 0.14.0: cp `src/landing/v5-sites.tsx` + `v5-sites-kit.tsx`
(новые) + `src/tokens.ts` (только версия-коммент), package.json
0.13.0→0.14.0 (+description-маркер; карта exports не менялась), dist
rebuild (локальный `dts: false`). **`src/landing/v5.tsx` НЕ копировался**
— вместо этого применён приложенный `v5.tsx.diff` поверх нашего
вендоренного файла (их база — чистый 0.13.0 без нашего `[vitrina] copy
patch` SEO-лида героя; копирование откатило бы прод-копи). 10 из 11
хунков применились как есть; последний (ре-экспорт `export * from
'./v5-sites'` в конце файла) дописан вручную — не сошёлся контекст
из-за финальной пустой строки, которую срезает наш pre-commit
(`fix end of files`). Итоговый дифф сверен построчно: затронуты только
импорт, `V5_CSS` (+`body.is-locked`), блок Examples/SiteViewer и хвост;
Hero/Header/Story/Pricing/Faq/Footer — byte-identical.

**НЕ копировались (наши локальные патчи, зип везёт upstream-состояние):**
`src/intake/in2.tsx` — converter-repair дублей `In2_CSS`/`In2_Styles`
в нижнем `export {}`-блоке (у них снова дубли — ESM-сборка падает
«Multiple exports»); `tsup.config.ts` — наш `dts: false` + entry
`src/feedback/index.tsx`. Оба расхождения повторно отправлены Claude
Design.

Проверка граблей конвертера по `CANON_V5_EXAMPLE_SITES_EXPORT_TZ.md §2`
пройдена: `'use client'` первой строкой обоих новых файлов, IIFE/`dcExport`
не утекли, `window.*` только SSR-safe guards (`typeof window !== 'undefined'`)
внутри хуков, self-ref констант нет, дублей экспортов нет, `React.lazy`
из прототипа заменён статическими импортами. Косметика: тело
`v5-sites.tsx` осталось с отступом в 2 пробела от канвас-обёртки —
парсится штатно, на сборку не влияет.

Консьюмер (controlled-провязка `viewerId`/`onViewerChange` + history/Back
+ Метрика-цели `example_site_open`) — отдельным PR.
