...
**Причины (7) — ЗАМОРОЖЕНО founder'ом 23.07.2026, коды консьерж-таблицы 1:1:**

| # | code | label |
|---|---|---|
| 1 | `enough_maps` | Мне хватает Яндекс.Карт и 2ГИС |
| 2 | `booking_covers` | Запись уже в Dikidi/YClients — зачем ещё сайт? |
| 3 | `unclear_value` | Не понял, что именно получу |
| 4 | `price` | Дорого |
| 5 | `no_trust` | Не доверяю: непонятно, кто вы |
| 6 | `not_now` | Пока просто смотрю — вернусь позже |
| 7 | `other` | Другое — напишу словами |

Проектные решения (зафиксированы): общий «не нужен» намеренно разложен на
`enough_maps` / `booking_covers` (безпричинное «не нужен» — ноль информации);
«просто смотрю» и «нет времени» слиты в один low-intent-код `not_now` — он же
фильтр шума от настоящих возражений.

Ноты исполнителям:
- **Канону:** при выборе `other` поле «Почему — одним предложением»
  становится ОБЯЗАТЕЛЬНЫМ (иначе other — мусорная корзина без содержимого);
  в остальных причинах — опциональное. Порядок радио фиксированный, как в
  таблице (primacy-смещение к первому пункту принято осознанно; кросс-проверка
  смещения — консьерж-канал, где порядка нет). Label 2 переносится на две
  строки на 375 — это учтено, короче нельзя без потери смысла.
- **Бэкенду (сделано):** enum заморожен в `core/feedback/service.py::
  FEEDBACK_V2_REASON_CODES`; БД-CHECK (миграция 0021) резервирует восьмое
  значение `no_reply` — существует ТОЛЬКО в консьерж-канале («молчание»),
  форма его не рендерит никогда, API отклоняет; когда консьерж-данные поедут
  в ту же аналитику, enum не придётся мигрировать. `other` без note → 400
  `note_required_for_other`.
- Один enum живёт в трёх местах 1:1 — форма, БД/Метрика, консьерж-таблица.
  Менять — только синхронно во всех трёх.
...
1. ~~Founder заполняет таблицу причин §4~~ — ✅ заморожено 23.07.2026.
