...
> **Сейчас:** canon 0.14.0 — «Примеры v2 · полноразмерные сайты за „Посмотреть сайт“» (23.07.2026): секция 03 лендинга v5 открывает **5 полноразмерных сайтов-примеров** (`FullComp`: Nail/Barber/Skin/Brow/ColoristSite — развитые версии миниатюр: полный прайс, галерея, отзывы, «о мастере», контакты, футер-watermark) в **takeover-просмотрщике во всю ширину** (`V5_SiteViewer`) с навигацией «← Все примеры» и всегда-доступным CTA. Новые файлы `src/landing/v5-sites{,-kit}.tsx` бандлятся в чанк `./landing`. Additive: `V5_Examples` получил controlled-API (`viewerId`/`onViewerChange`), закрытое состояние = 0.13.0 + хуки `data-example-site/-open/-back`. Фикс: `body.is-locked` впервые получил CSS-правило (до 0.14.0 класс вешался, но правила не было — фон скроллился под просмотром). Консьюмер-фикс в том же PR: `body.is-locked [data-fb-fab]{display:none}` в `globals.css` — наш фидбек-FAB (z-190) перекрывал CTA просмотрщика (z-120) на мобиле и съедал тапы по кнопке конверсии. Предыдущее состояние: canon 0.13.0 — «Фидбек v2 · Что останавливает?» (июль 2026): vote-first `S9_FeedbackModal` **удалён** из canon (breaking, без депрекейта), замена — новый модуль `@samosite/canon/feedback` (`FeedbackV2Modal`: режим blocker «Что останавливает прямо сейчас?» с exit-intent/скролл-60% авто-триггером + режим question «Задать вопрос»; `FeedbackV2Fab`, `REASONS` = замороженный enum 7 причин консьерж-таблицы). Консьюмер `landing/components/FeedbackModal.tsx` переключён атомарно (PR #261): показ-логика 1-раз-на-посетителя, SmartCaptcha → `POST /api/feedback/v2` (миграция 0021 CHECK enum), Метрика-цели `feedback_open{trigger}`/`feedback_reason`/`feedback_contact_left`/`feedback_question_sent` (см. runbook). `GET /api/feedback/tally` + votes-ветка — **ретайрнуты** (июль 2026; таблицы 0012 read-only, история в админ-инбоксе). Между 0.11.0 и 0.13.0 был 0.12.0 «Витрина v5 · Фарфор и лак» (ребренд токенов + `V5_*` лендинг + `In2_*` интейк — см. ⚠-блок ниже, главная на v5). Предыдущее состояние: canon 0.11.0 — dual vendor 0.9.5→0.11.0 (0.10.0 + 0.11.0 одним PR; только `src/intake/*` + `src/primitives`): 0.10.0 = instant-preview rev.1 — `S3_StepBuilding`/`S3_StepPreview` (rich/sparse), `PreviewDraft`, controlled, opt-in через проп `preview` у SubmitModal; 0.11.0 = rev.2 «ниша-демо» — `S3_StepNiche`/`S3_StepSource`, `NICHE_LIB`, поиск по названию, морф «пример → черновик», opt-in через проп `entry`; `Btn` — additive `data-*` passthrough. **Фронт-консьюмер подключён** (`feat/preview-consumer`): `landing/components/SubmitModal.tsx` передаёт `entry`/`preview` — пустой hero-инпут → ниша → пример → источник → морф-сборка → превью; hero-URL → классический 0.3.0-флоу без изменений. Бэкенд `GET /api/preview/search` + `POST /api/preview/draft` (клиент `landing/lib/preview-api.ts`, контракт: `CANON_INSTANT_PREVIEW_REV2_TZ.md §7`) ещё не построен — до него поиск деградирует в «Карты не отвечают», сборка → failed → контакт (rev.1 fallback); 7 новых Метрика-goals `intake_*` (см. `docs/runbooks/yandex-metrika-goals.md`). 0.9.5 (PATCH, только `src/customer/index.tsx`): фикс `S9_FeedbackModal` — вложенные `Dialog`/`FauxPage`/`FloatingBtn` вызываются как функции (`{Dialog()}`), а не монтируются как `<Dialog/>`; иначе React пересоздавал поддерево на каждый клик/ввод (скролл прыгал вверх, инпут терял фокус). Публичный API/пропсы без изменений → консьюмер `FeedbackModal.tsx` не трогали, drift=0. 0.9.4 (PATCH) = version-sync перевыпуск 0.9.3: **идентичный код**, бамп номера из-за коллизии версий на npm-пайплайне канон-команды (их опубликованный `0.9.3` не подхватил landing-фиксы). У нас 0.9.3 уже вёз фиксы (вендорим `src/` напрямую, не из их реестра) — на проде `/version` 541efe3 с `ss-nav-link`×9. 0.9.4 = только бамп `package.json` + CHANGELOG, `dist` байт-в-байт, редеплой не нужен. 0.9.3 (PATCH, только `src/landing/index.tsx`): `StickyHeader` унифицировал hover навигации (класс `.ss-login-link` **удалён** → все пункты, включая «Войти», используют `.ss-nav-link`; hover-правила с `!important`, чтобы переживать инлайн-запекание при экспорте артборда), `HeroBlock` абзац-2 переведён с `text-wrap: pretty` на `balance` (убран висячий хвост «новых заявок»). Консьюмер `SiteHeader` репойнт «Войти» → `/login` переведён с селектора `.ss-login-link` на устойчивый `a[href="#login"]`; разметка/экспорты без изменений, drift=0. 0.9.2 (PATCH, только `src/presets/index.tsx`): превью клиентских сайтов в Examples (блок 02) выросли 460→720px и стали полноценными многоблочными страницами (5 семейств: статистика, прайс, отзыв, футер-CTA); drift=0, авто-подхват (`MiniChrome` без явного `height`). Landing-разметка не менялась с 0.8.0 (10 секций через canon); 0.9.1 перевёл `S9_FeedbackModal` на **controlled API** (`open`/`onOpenChange`/`tally`/`onSubmit`/`submitting`/`error`/`embedded` + 5 TS-типов; zero-prop = canvas-mock). Раскатка завершена: PR1 (вендор 0.9.0) + PR3 (backend votes/tally + миграция 0012) + PR-A (вендор 0.9.1 controlled) + PR-B (консьюмер) — все merged. Feedback на проде = глобальная canon-модалка `<FeedbackModal>` (тонкий адаптер `tally`←GET / `onSubmit`→POST / `embedded={false}`, монтируется в `app/layout.tsx`, self-hide на `/admin*`+`/login`); `/feedback` ретайрнут (301→`/`), hand-rolled `FeedbackForm` удалён. **Landing (0.8.0, без изменений):** Examples → Cycle → Monday → BaseWork → Sources → Analytics → Ownership → Pricing → FAQ → FinalCta (Analytics над Ownership, в каноне). 0.8.0: `ExamplesSection` переписан как фикс-высотная (460px) **карусель превью** (`ExamplesCarousel`) + блок `HowItPicks` «Дизайн собирается из ваших материалов»; `PricingSection` = 5-тарифная `PricingMatrix` (490/990 модель убрана окончательно); `BaseWorkSection` (чистые редакционные карточки) и `OwnershipSection` (2×2 мини-контролы, перенесён под аналитику, без «Демо ЛК») переверстаны; у Я.Карт снят бейдж «ЧАЩЕ ВСЕГО»; хедер-навигация = Примеры · Цена · Помощь · Войти. **Vendoring-фикс:** upstream 0.8.0 preview→package конвертер протёк (leaked IIFE opener + self-ref `const PresetRenderer = PresetRenderer`) — 2 минимальных правки в `src/landing/index.tsx`, залогировано в CHANGELOG 0.8.0 + отправлено Claude Design. Консьюмерские хаки (`SiteHeader` href-override, `CanonCtaBindings` ownership-demo-removal → no-op, `/feedback` для «Не нашли свою») сверены с 0.8.0-разметкой — работают без изменений. Hand-rolled Hero синхронизирован с каноном 0.7.2 (абзацы 1/2, `<ChipStrip>` импортом) — копи Hero в 0.8.0 не менялась. Композиция `app/page.tsx` не менялась. Canvas mirror + baselines остались от 0.6.0 (smoke-only, не enforced). `/admin-demo` shipped (PR #122). `/customer-demo` shipped (PR #124). Hero (interactive), `*.samosite.online` customer sites, admin chrome, intake — остаются hand-rolled (см. `CANON_SWAP_PLAN.md` для пути forward).
...
| 9 | FeedbackV2Modal (global, canon 0.13.0 — blocker + question) | 🔵 | 🔵 | 🔵 | 🔵 | `CANON_FEEDBACK_V2_TZ.md` |
