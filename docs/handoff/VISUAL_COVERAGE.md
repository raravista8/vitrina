...
> **Сейчас:** canon 0.7.3 — landing рендерит 10 секций через canon (Examples → Cycle → Monday → BaseWork → Sources → Analytics → Ownership → Pricing → FAQ → FinalCta — Analytics и Ownership поменяны местами по продуктовому запросу), drift = 0 by construction. 0.7.1 переписал `ExamplesSection` (карусель из 10 пресет-карточек + блок «Как ИИ подбирает стиль») и `PricingSection` (5-тарифная `PricingMatrix`), добавил модуль `@samosite/canon/presets`. 0.7.2 — фикс респонсива хедера (CTA не обрезается 720–1000px) + `ChipStrip` отдельным экспортом + Hero абзац 2 переписан. Hand-rolled Hero синхронизирован с каноном 0.7.2 (абзацы 1/2, убран photo-link, добавлен `<ChipStrip>` импортом) — см. `DESIGN_SPEC.md` + чек-лист. Композиция `app/page.tsx` не менялась. Canvas mirror + baselines остались от 0.6.0 (smoke-only, не enforced). `/admin-demo` shipped (PR #122). `/customer-demo` shipped (PR #124). Hero (interactive), `*.samosite.online` customer sites, admin chrome, intake — остаются hand-rolled (см. `CANON_SWAP_PLAN.md` для пути forward).
