...
> **Сейчас:** canon 0.7.1 — landing рендерит 10 секций через canon (Examples → Cycle → Monday → BaseWork → Sources → Ownership → Analytics → Pricing → FAQ → FinalCta), drift = 0 by construction. 0.7.1 переписал `ExamplesSection` (карусель из 10 пресет-карточек + блок «Как ИИ подбирает стиль») и `PricingSection` (5-тарифная `PricingMatrix` вместо одиночной карточки), добавил модуль `@samosite/canon/presets`. Композиция `app/page.tsx` не менялась — те же 10 экспортов. Canvas mirror + baselines остались от 0.6.0 (smoke-only, не enforced — см. ниже). `/admin-demo` shipped (PR #122). `/customer-demo` shipped (PR #124). Hero (interactive), `*.samosite.online` customer sites, admin chrome, intake — остаются hand-rolled (см. `CANON_SWAP_PLAN.md` для пути forward).
