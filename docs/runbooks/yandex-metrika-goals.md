...
>
> **СТАТУС Примеры v2: выполнено 23.07.2026** — под canon 0.14.0
> («Примеры v2», полноразмерные сайты-примеры) заведены 2 цели:
> `example_site_open` (ID 588170648) и `example_site_back` (ID 588170722).
> Проводка — `landing/components/V5Landing.tsx` (controlled-просмотрщик).
>
> **СТАТУС Feedback v2: выполнено 23.07.2026** — под canon 0.13.0
> («Фидбек v2», ADR см. `docs/handoff/CANON_FEEDBACK_V2_TZ.md`) заведены
> 3 новые цели (см. раздел ниже). `feedback_open` переиспользована — у неё
> сменилась семантика параметра (`trigger` вместо `source`). Vote-first
> `feedback_submit` (ID 587310685) больше не срабатывает — не удалять.
...
### Feedback v2 (canon 0.13.0, июль 2026) — заведены 23.07.2026

Проводка: `landing/components/FeedbackModal.tsx` (адаптер `FeedbackV2Modal`).
Тип «Целевое событие», оператор «Совпадает».

| Goal ID | UI ID | Триггер | Параметры |
|---|---|---|---|
| `feedback_open` | 563344844 (переиспользована) | открылась модалка (любой режим) | `trigger`: exit \| scroll \| button — сменил старый `source` |
| `feedback_reason` | 588105168 | выбрана причина в блокере | `reason`: 7 кодов консьерж-таблицы (enum заморожен, `FEEDBACK_V2_REASON_CODES`) |
| `feedback_contact_left` | 588107899 | блокер: оставил контакт на черновик (мини-лид) | `channel`: telegram \| whatsapp \| email |
| `feedback_question_sent` | 588108065 | режим «Задать вопрос»: вопрос отправлен (202) | — |

### Примеры v2 (canon 0.14.0, июль 2026) — заведены 23.07.2026

Проводка: `landing/components/V5Landing.tsx` — просмотрщик сайтов-примеров
в controlled-режиме (`viewerId`/`onViewerChange`). Тип «Целевое событие»,
оператор «Совпадает».

| Goal ID | UI ID | Триггер | Параметры |
|---|---|---|---|
| `example_site_open` | 588170648 | открыл полноразмерный сайт-пример («Посмотреть сайт») | `example`: nails \| barber \| skin \| brows \| colorist |
| `example_site_back` | 588170722 | вернулся в карусель (кнопка «Все примеры», Esc, × или **браузерный Back**) | — |

Клик «Собрать такой же за 2 часа» **из просмотрщика** отдельной цели не
имеет — его ловит существующая `form_open` с `entry="example-<id>"`
(canon вешает `data-entry` на CTA просмотрщика). Воронка примеров:
`example_site_open` → `form_open{entry:example-*}` → `submit_success`.
...
| `feedback_open` | фидбек-модалка | Feedback v2 — см. таблицу выше (параметр `trigger`) |
| `feedback_submit` | голоса отправлены | **затихла** — vote-first модалка удалена в canon 0.13.0; не удалять (история) |
