// Самосайт · Landing — canvas mirror for visual-regression baseline gen.
// Generated from canon 0.6.0 src/landing/index.tsx (~2315 lines).
// Imports replaced with window-destructure; export block replaced with
// Object.assign(window, …). Babel-standalone reads JSX directly; no TS
// features are used in canon 0.6.0 source so no transpilation needed
// beyond JSX → JS.
//
// Sync to canon 0.6.0:
//   1. Edit `packages/canon/src/landing/index.tsx` (when canon updates).
//   2. Re-run `python3 infra/scripts/regen-canvas-mirror.py` (this transform).
//   3. `bash infra/scripts/generate-canon-baselines-linux.sh` to regen PNGs.

if (!window.__ss_landing) { window.__ss_landing = true; (function ssLanding(){

const VT = window.VT;
const { BRAND, Btn, IconArrow, IconLink, BrandMark } = window;
