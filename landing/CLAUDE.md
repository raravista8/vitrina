...
- `landing/components/V5Landing.tsx` — секция «Примеры» (`V5_Examples`, canon 0.14.0): клик «Посмотреть сайт» открывает полноразмерный сайт-пример в takeover-просмотрщике (`V5_SiteViewer` + `FullComp` × 5). Пока uncontrolled (стейт вьюера внутри канона); controlled-режим `viewerId`/`onViewerChange` + browser-Back — следующим PR. Консьюмер-правило в `globals.css`: `body.is-locked [data-fb-fab]{display:none}` — прячет фидбек-FAB на время просмотра (иначе перекрывает CTA просмотрщика на мобиле)
